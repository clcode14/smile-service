...
import org.flightythought.smile.appserver.bean.LoginLimitData;
...
public class LocalCache {
    ...
    public static boolean hasMoreLoginFailRequest(String req) {
        ...
        if (loginLimitData != null) {
            if (loginLimitData.getCount() >= 10 && (now - loginLimitData.getLoginTime()) <= 30 * 60 * 1000) {
                //手机登陆失败10次，30分钟内不允许登陆
                return true;
            } else {
